/**
 * Regenerate only the autopilot icon with new creative concept
 */
...
// New creative autopilot prompt - different concept
const AUTOPILOT_PROMPT = `Subject: A glowing autonomous robot pilot figure with neural network connections and flight path indicators, representing AI automation and intelligent navigation.
Colors: blue and cyan gradient (#3B82F6 to #06B6D4)
Style: Intelligent automation, AI-driven, autonomous systems, smart navigation
${BASE_STYLE}`;
...
async function regenerateAutopilotIcon(): Promise<boolean> {
  console.log("🔄 Regenerating autopilot icon with new creative concept...");
  console.log("New concept: Autonomous robot pilot with neural connections");
...
Now create a NEW icon with this exact style:\n\n`,
  ...
  contents.push({ text: AUTOPILOT_PROMPT });
    ...
    if (response.candidates?.[0]?.content?.parts) {
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData?.mimeType?.startsWith("image/")) {
          console.log("✓ Generated new autopilot icon");

          // Save temporarily
          const tempFilename = "header-dashboard-autopilot-temp.png";
          ...
          console.log(
            `✓ Saved new autopilot icon (${transparentBuffer.length} bytes)`,
          );
...
async function main() {
  const success = await regenerateAutopilotIcon();

  if (success) {
    console.log("\n✅ Autopilot icon regenerated successfully!");
    console.log(
    ...
    console.log(
      "🎨 New concept: Autonomous robot pilot with neural connections",
    );
  } else {
    console.log("\n❌ Failed to regenerate autopilot icon");
  }
}
