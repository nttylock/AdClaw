/**
 * Regenerate only the terms icon with new prompt
 */
...
// New terms prompt
const TERMS_PROMPT = `Subject: A glowing shield with checkmark symbol inside, representing legal protection and agreement acceptance.
...
async function regenerateTermsIcon(): Promise<boolean> {
  console.log("🔄 Regenerating terms icon with new prompt...");
  console.log("New concept: Glowing shield with checkmark");
...
Now create a NEW icon with this exact style:\n\n`,
  ...
  contents.push({ text: TERMS_PROMPT });
    ...
    if (response.candidates?.[0]?.content?.parts) {
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData?.mimeType?.startsWith("image/")) {
          console.log("✓ Generated new terms icon");

          // Save temporarily
          const tempFilename = "header-terms-temp.png";
          ...
          console.log(
            `✓ Saved new terms icon (${transparentBuffer.length} bytes)`,
          );
          return true;
        }
      }
...
async function main() {
  const success = await regenerateTermsIcon();

  if (success) {
    console.log("\n✅ Terms icon regenerated successfully!");
    console.log("📍 Location: /public/new-icons/header-terms.png");
  } else {
    console.log("\n❌ Failed to regenerate terms icon");
  }
}
