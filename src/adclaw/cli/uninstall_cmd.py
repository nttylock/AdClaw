# -*- coding: utf-8 -*-
"""adclaw uninstall — remove the AdClaw environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove AdClaw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# AdClaw" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# AdClaw\nexport PATH=\"\$HOME/\.adclaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove AdClaw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL AdClaw data in {wd}")
    else:
        click.echo(
            "This will remove the AdClaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("AdClaw uninstalled. Please restart your terminal.")
