...
在于AdClaw对话前，需要先配置模型。在 **控制台 → 设置 → 模型** 中可以快捷配置。

![控制台模型](https://img.alicdn.com/imgextra/i4/O1CN01XnOPPQ1c99vox3I88_!!6000000003557-2-tps-3786-1980.png)

AdClaw 支持多种 LLM 提供商：**云提供商**（需 API Key）、**本地提供商**（llama.cpp / MLX）和 **Ollama 提供商**，且支持添加自定义 **提供商**。本文介绍这几类提供商的配置方式。
...
Ollama 提供商对接本机安装的 **Ollama 守护进程**，使用其中的模型，无需由 AdClaw 直接下载模型文件，列表会与 Ollama 自动同步。

**前置条件：**

- 从 [ollama.com](https://ollama.com) 安装 Ollama。
- 在 AdClaw所在虚拟环境中安装 Ollama：`pip install 'adclaw[ollama]'`。
...
> 如果在过程中遇到 `Ollama SDK not installed. Install with: pip install 'adclaw[ollama]'`的提示，请先确认是否已经在 ollama.com 下载 Ollama，并在 AdClaw所在虚拟环境中执行过 `pip install 'adclaw[ollama]'`。如果想删除某个模型，点击 Ollama 卡片右下角的 **模型**，在模型列表中，点击想要删除的模型右侧的 **垃圾桶按钮**，二次确认后即可删除。
>
