...
- **`~/.adclaw`** (the `.adclaw` folder under your home directory)

When you run `adclaw init`, this directory is created automatically. Here's what
you'll find inside:
...
| `jobs.json`          | Cron job list (managed via `adclaw cron` or API)                    |
...
> generic "You are a helpful assistant" prompt. Run `adclaw init` to auto-copy
...
If you don't want to use `~/.adclaw`, you can override the working directory or
specific file names:

| Variable                           | Default         | Meaning                                                                             |
| ---------------------------------- | --------------- | ----------------------------------------------------------------------------------- |
| `ADCLAW_WORKING_DIR`                | `~/.adclaw`      | Working directory; config, heartbeat, jobs, chats, skills, and memory all live here |
| `ADCLAW_CONFIG_FILE`                | `config.json`   | Config file name (relative to working dir)                                          |
| `ADCLAW_HEARTBEAT_FILE`             | `HEARTBEAT.md`  | Heartbeat prompt file name (relative to working dir)                                |
| `ADCLAW_JOBS_FILE`                  | `jobs.json`     | Cron jobs file name (relative to working dir)                                       |
| `ADCLAW_CHATS_FILE`                 | `chats.json`    | Chats file name (relative to working dir)                                           |
| `ADCLAW_LOG_LEVEL`                  | `info`          | Log level for the app (`debug`, `info`, `warning`, `error`, `critical`)             |
| `ADCLAW_MEMORY_COMPACT_THRESHOLD`   | `100000`        | Character threshold to trigger memory compaction                                    |
| `ADCLAW_MEMORY_COMPACT_KEEP_RECENT` | `3`             | Number of recent messages kept after compaction                                     |
| `ADCLAW_MEMORY_COMPACT_RATIO`       | `0.7`           | Threshold ratio for triggering compaction (relative to context window)              |
| `ADCLAW_CONSOLE_STATIC_DIR`         | _(auto-detect)_ | Path to the console front-end static files                                          |

Example — use a different working dir for this shell:

```bash
export ADCLAW_WORKING_DIR=/home/me/my_adclaw
adclaw app
```
...
| `media_dir`          | string | `~/.adclaw/media` | Directory for received media files  |
...
| `agents.installed_md_files_language` | string \| null | `null`    | Tracks which language's MD files are installed; managed by `adclaw init` |
...
| Field  | Type           | Default | Description                   |
| ------ | -------------- | ------- | ----------------------------- |
| `host` | string \| null | `null`  | Last host used by `adclaw app` |
| `port` | int \| null    | `null`  | Last port used by `adclaw app` |

This is auto-saved every time you run `adclaw app`. Other CLI subcommands
(like `adclaw cron`) use this to know where to send requests.
...
AdClaw needs an LLM provider to work. You can set it up in three ways:

- **`adclaw init`** — interactive wizard, the easiest way
...
> **Tip:** Run `adclaw init` and follow the prompts — it will list available
...
> AdClaw does not verify whether the key is correct or has sufficient quota —
...
- **`adclaw init`** — prompts "Configure environment variables?" during setup
...
> keys) are valid. AdClaw only stores and injects them — it does not verify
> correctness.
...
| Directory                     | Purpose                                                             |
| ----------------------------- | ------------------------------------------------------------------- |
| Built-in (in source code)     | Shipped with AdClaw — docx, pdf, pptx, xlsx, news, email, cron, etc. |
| `~/.adclaw/customized_skills/` | User-created skills                                                 |
| `~/.adclaw/active_skills/`     | Currently active skills (synced from built-in + customized)         |
...
- `adclaw init` (choose all / none / custom during setup)
- `adclaw skills config` (interactive toggle)
...
AdClaw has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention. See [Memory](./memory.en.md) for full details.

Memory files are stored in two locations:

| File / Directory                | Purpose                                                               |
| ------------------------------- | --------------------------------------------------------------------- |
| `~/.adclaw/MEMORY.md`            | Long-lived key information (decisions, preferences, persistent facts) |
| `~/.adclaw/memory/YYYY-MM-DD.md` | Daily logs (notes, runtime context, auto-generated summaries)         |
...
- Everything lives under **`~/.adclaw`** by default; override with
  `ADCLAW_WORKING_DIR` (and related env vars) if needed.
...
- LLM providers are configured via `adclaw init` or the console UI.
