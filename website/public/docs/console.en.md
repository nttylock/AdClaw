...
The **Console** is AdClaw's built-in web interface. After running `adclaw app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with AdClaw in real time
- Enable/disable messaging channels
- View and manage all chat sessions
- Manage scheduled jobs
- Edit AdClaw's persona and behavior files
- Enable/disable skills to extend AdClaw's capabilities
- Manage MCP clients
...
This is where you talk to AdClaw. It is the default page when the Console
opens.
...
button (↑). AdClaw replies in real time.
...
Create and manage scheduled jobs that AdClaw runs automatically by time.
...
2. Fill in each section:
   ...
   - **Task Type & Content** — **Text** (fixed message) or **Agent** (ask
     AdClaw and forward reply), then the content
...
Edit files that define AdClaw's persona and behavior, such as `SOUL.md`,
...
Manage skills that extend AdClaw's capabilities (for example: PDF reading,
...
Configure LLM providers and choose the model AdClaw uses. AdClaw supports both
...
first (`pip install 'adclaw[llamacpp]'` or `pip install 'adclaw[mlx]'`).
...
- Models come from the Ollama daemon (not downloaded directly by AdClaw)
- Model list is auto-synced with Ollama
- Popular model examples: `mistral:7b`, `qwen3:8b`

> You can also manage Ollama models via CLI: `adclaw models ollama-pull`,
> `adclaw models ollama-list`, `adclaw models ollama-remove`. See
...
> **Note:** Cloud API key validity is your responsibility. AdClaw does not
...
Manage runtime environment variables needed by AdClaw tools and skills
...
> **Note:** Variable validity is your responsibility. AdClaw only stores and
> loads values.
>
...
| --------------------- | -------------------------------- | ---------------------------------------------------- |
| Chat                  | Chat → Chat                      | Talk with AdClaw, manage sessions                     |
