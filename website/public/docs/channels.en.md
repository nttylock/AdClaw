...
A **channel** is where you talk to AdClaw: connect DingTalk and it replies
...
- **Edit `config.json` directly** — Default `~/.adclaw/config.json` (created by `adclaw init`), set `enabled: true` and fill in that platform's credentials. Saving triggers a reload without restarting the app.
...
7. (Optional) **Add your server's IP to the whitelist** — this is required for features that call the DingTalk Open API (e.g. downloading images and files sent by users). Go to **"Security & Compliance → IP Whitelist"** in your app settings and add the public IP of the machine running AdClaw. You can find your public IP by running `curl ifconfig.me` in a terminal. If the IP is not whitelisted, image and file downloads will fail with a `Forbidden.AccessDenied.IpNotInWhiteList` error.

### Link the app

You can configure it either in the Console frontend or by editing `~/.adclaw/config.json`.
...
**Method 2**: Edit `~/.adclaw/config.json`
...
4. Run **`adclaw app`** to start AdClaw
...
> **Note:** Follow this order: Configure App ID/Secret → start `adclaw app` → then configure the long connection in the Feishu console. If errors persist, try stopping the adclaw service and restarting `adclaw app`.
...
Find `channels.feishu`（default as `~/.adclaw/config.json`） in `config.json`. Only **App ID** and **App Secret** are required (copy from the Feishu console under Credentials & basic info):
...
Other fields (encrypt_key, verification_token, media_dir) are optional; with WebSocket mode you can omit them (defaults apply). Then `pip install lark-oapi` and run `adclaw app`. If your environment uses a SOCKS proxy, also install `python-socks` (for example, `pip install python-socks`), otherwise you may see: `python-socks is required to use a SOCKS proxy`.

> **Note:** You can also fill in **App ID** and **App Secret** in the Console UI, but you must restart the adclaw service before continuing with the long-connection configuration.
...
You can configure via the Console UI or by editing `~/.adclaw/config.json`.
...
from adclaw.app.channels.base import BaseChannel
from adclaw.app.channels.schema import ChannelType
...
- **Directory**: Channels under the working dir at `custom_channels/` (default `~/.adclaw/custom_channels/`) are loaded at runtime. The manager scans `.py` files and packages (subdirs with `__init__.py`), loads `BaseChannel` subclasses, and registers them by the class’s `channel` attribute.
- **Install**: `adclaw channels install <key>` creates a template `<key>.py` in `custom_channels/` for you to edit, or use `--path <local path>` / `--url <URL>` to copy a channel module from disk or the web. `adclaw channels add <key>` does the same and also adds a default entry to config (with optional `--path`/`--url`).
- **Remove**: `adclaw channels remove <key>` deletes that channel’s module from `custom_channels/` (custom channels only; built-ins cannot be removed). By default it also removes the key from `channels` in `config.json`; use `--keep-config` to leave config unchanged.
- **Config**: `ChannelConfig` uses `extra="allow"`, so any channel key can appear under `channels` in `config.json`. Use `adclaw channels config` for interactive setup or edit config by hand.
